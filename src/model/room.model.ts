...
export async function roomGetOnlyMembers(room_id: number) {
    return query<{id: string, name: string}[]>('SELECT id, name FROM users WHERE id IN (SELECT user_id from seats WHERE room_id=$1 AND author=false);', [room_id])
}
...
export async function syncRoomMembers(roomId: number, users: number[]) {
    ...
    let added = -1;
    if(toDel.length) {
        const places = Object.keys(toDel).map(k=>'$'+(2-(-k))).join(', ');
        const sql = `DELETE FROM seats WHERE room_id=$1 AND user_id IN (${places});`;
        const result = await query<{rowCount:number}>(sql, [roomId, ...toDel], {raw: true});
